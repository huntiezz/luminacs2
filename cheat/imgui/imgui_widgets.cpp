...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(full_width, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    ImVec2 check_pos = ImVec2(pos.x + total_bb.GetWidth() - square_sz, pos.y + (total_bb.GetHeight() - square_sz) * 0.5f);
    ImRect check_bb(check_pos, check_pos + ImVec2(square_sz, square_sz));
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_Separator), style.FrameRounding);
    ...
    if (mixed_value)
    {
        ...
        window->DrawList->AddRect(check_bb.Min + pad, check_bb.Max - pad, GetColorU32(ImGuiCol_Separator), style.FrameRounding);
    }
    else if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(48, 85, 255, 255));
    }

    ImVec2 label_pos = ImVec2(pos.x, pos.y + (total_bb.GetHeight() - label_size.y) * 0.5f);
    if (g.LogEnabled)
